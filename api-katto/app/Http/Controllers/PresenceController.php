...
class PresenceController extends Controller
{
    ...
    {
        $presences = Presence::with('employee')->whereDate('datetime', date('Y-m-d'))->latest()->paginate(9);
    ...
    public function show(Employee $employee)
    {
        if (request()->has('from') && request()->has('to')) {
            return PresenceResource::collection($employee->presences()->whereBetween('datetime', [request()->from, request()->to." 23:59:59"])->paginate(9));
        }
